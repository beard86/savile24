...
var _converter = require('sass-convert');

var _converter2 = _interopRequireWildcard(_converter);
...
var Environment = (function (_EventEmitter) {
  ...
  function Environment(logger) {
    ...
    this.on('error', function (error) {
      var friendlyErrors = [errors.SassDocError, _converter2['default'].BinaryError, _converter2['default'].VersionError];
      ...
      if (_Array$find(friendlyErrors, function (c) {
        ...
        if (_is.is.error(error) && 'stack' in error) {
  ...
  Environment.prototype.load = function load(config) {
    ...
    if (_is.is.string(config)) {
      return this.loadFile(config);
    }

    if (_is.is.plainObject(config)) {
  ...
  Environment.prototype.parseFile = function parseFile(file) {
    return _yaml2['default'].safeLoad(_fs2['default'].readFileSync(file, 'utf-8'));
  };
  ...
  Environment.prototype.resolve = function resolve(file) {
    var cwd = arguments[1] === undefined ? false : arguments[1];
